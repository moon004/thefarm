...
func (tf *TheFarm) translateChar(C *TheChar) {
	...
	if !AlmostEq(CX, C.CD.X, tol) {
		C.CN.SetPositionX(CX + dX) // Move node
	} else {
		C.CO.X = C.CD.X
		C.CD.X = tf.randPoint(-2, 8)
	}
	if !AlmostEq(CZ, C.CD.Z, tol) {
		C.CN.SetPositionZ(CZ + dZ)
	} else {
		C.CO.Z = C.CD.Z
		C.CD.Z = tf.randPoint(-8, 6)
	}

	rad := math32.Atan2(distX, distZ) - math32.Pi/2
	// rad := float32(0)
	...
	log.Debug("\nCurrentPos: %v, CurrentDes: %v, Radian: %v, CRotation: %v, DetectQuad: %v",
		C.CN.Position(), C.CD, rad, CurrentRot, quad)
}

// DetectQuad returns the quadrant number that
// the current coordination is at. Returns
// 0 if at origin.
func DetectQuad(z, x float32) int {
	// Check if first quadrant
	switch {
	case z < 0 && x > 0: // first quadrant
		return 1
	case z < 0 && x < 0: // second quadrant
		return 2
	case z > 0 && x < 0: // third quadrant
		return 3
	case z > 0 && x > 0: // fourth quadrant
		return 4
	default:
		return 0
	}
